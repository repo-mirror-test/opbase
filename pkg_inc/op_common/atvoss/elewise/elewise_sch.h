 ...
 * \brief
 */
#ifndef ELEWISE_SCH_H_
#define ELEWISE_SCH_H_
...
class ElementwiseSch {
 ...
 protected:
  ...
  __aicore__ inline void CopyIn(uint64_t offset, uint64_t tileLength, int32_t pingPong) {
    ...
    LocalTensor<TensorType> inTensor = tensorPool[bufId * blockLen].template ReinterpretCast<TensorType>();
    inTensor.SetBufferLen(blockLen / sizeof(TensorType));
