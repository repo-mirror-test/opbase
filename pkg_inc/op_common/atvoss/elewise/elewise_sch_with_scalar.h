...
/*!
 * \file elewise_sch_with_scalar.h
 * \brief
 */
#ifndef ELEWISE_SCH_WITH_SCALAR_H_
#define ELEWISE_SCH_WITH_SCALAR_H_
...
class ElementwiseSchWithScalar {
 public:
  ...
  __aicore__ inline void Init(Args... args) {
    static_assert(inputNums + outputNums == sizeof...(Args),
                  "ElementwiseSchWithScalar.Init args num should match DAG holders.");
  ...
  __aicore__ inline void SetScalar(int offset) {
    if constexpr (idx < ElemDag::VarSize) {
      ...
      scalars.template Set<idx>(reinterpret_cast<__tiling_data_ptr__ DType&>(tilingData->scalarData[offset]));
 ...
 protected:
  ...
  __tiling_data_ptr__ EleBaseTilingDataStruct* tilingData;
...
#endif  // ELEWISE_SCH_WITH_SCALAR_H_
