...
TEST_F(CommonTypesTest, TestAclScalarCheckOverflowsQint32ToOther)
{
    try {
        int32_t intValueMaxInt32(std::numeric_limits<int32_t>::max());
        aclScalar qintScalarMaxInt32(&intValueMaxInt32, op::DataType::DT_QINT32);
        EXPECT_TRUE(qintScalarMaxInt32.CheckOverflows<int32_t>());
...
TEST_F(CommonTypesTest, CreateContiguousUNWSTensor002)
{
    ...
    EXPECT_EQ(a.GetTensor()->GetSize(), 2048);
    ...
    a.SetDataType(op::DataType::DT_INT32);
    EXPECT_EQ(a.GetTensor()->GetSize(), 2048);
    a.SetDataType(op::DataType::DT_FLOAT16);
    EXPECT_EQ(a.GetTensor()->GetSize(), 1024);
...
TEST_F(CommonTypesTest, TestAclIntArrayToAclTensor)
{
    ...
    EXPECT_NE(ToString(array).GetString(), nullptr);
...
TEST_F(CommonTypesTest, aclTensorToString)
{
    aclTensor* self = new aclTensor(op::DataType::DT_FLOAT, op::Format::FORMAT_NHWC, op::Format::FORMAT_ND);
    EXPECT_NE(self->ToString().GetString(), nullptr);
    EXPECT_NE(ToString(*self).GetString(), nullptr);
    EXPECT_NE(ToString(self).GetString(), nullptr);
    aclDataType dtype = ACL_FLOAT;
    EXPECT_NE(ToString(dtype).GetString(), nullptr);
    delete self;
    aclTensor* self1 = nullptr;
    EXPECT_NE(ToString(self1).GetString(), nullptr);
}
TEST_F(CommonTypesTest, aclTensorListToString)
{
    op::Shape wsShape{32};
...
TEST_F(CommonTypesTest, aclScalarToString)
{
    ...
    EXPECT_NE(scalar.ToString().GetString(), nullptr);
}
...
{
    float fpValue = 3.2;
    auto floatArray = aclArray<float>(&fpValue, 1);
    EXPECT_NE(floatArray.ToString().GetString(), nullptr);
    auto floatArrayValue = floatArray[0];
    auto aclfloatArray = aclFloatArray(&fpValue, 1);
    EXPECT_NE(ToString(&aclfloatArray).GetString(), nullptr);

    int64_t int64Value = 1;
    auto int64Array = aclArray<int64_t>(&int64Value, 1);
    EXPECT_NE(int64Array.ToString().GetString(), nullptr);
    auto aclint64Array = aclIntArray(&int64Value, 1);
    EXPECT_NE(ToString(&aclint64Array).GetString(), nullptr);

    bool boolValue = true;
    auto boolArray = aclArray<bool>(&boolValue, 1);
    EXPECT_NE(boolArray.ToString().GetString(), nullptr);
    auto aclboolArray = aclBoolArray(&boolValue, 1);
    EXPECT_NE(ToString(&aclboolArray).GetString(), nullptr);

    op::fp16_t fp16Value = 3.2;
    auto fp16Array = aclArray<op::fp16_t>(&fp16Value, 1);
    EXPECT_NE(fp16Array.ToString().GetString(), nullptr);
    auto aclfp16Array = aclFp16Array(&fp16Value, 1);
    EXPECT_NE(ToString(&aclfp16Array).GetString(), nullptr);

    op::bfloat16 bfp16Value = 3.2;
    auto bfp16Array = aclArray<op::bfloat16>(&bfp16Value, 1);
...
TEST_F(CommonTypesTest, aclTensorCreate)
{
    float fpValue = 3.2;
    uint64_t size = 1;
...
{
    aclTensor tensor({1, 2, 3}, DataType::DT_FLOAT, Format::FORMAT_ND, nullptr);
    const int64_t arr[] = {1, 2, 3};
