...
if test -d "$CANN_PATH/ops_base" -a test -d "$CANN_PATH/../latest"
    set INSATLL_PATH (cd (dirname $REAL_SHELL_PATH)/../../../ && pwd)
    if test -L "$INSATLL_PATH/latest/ops_base"
        set _ASCEND_OPS_BASE_PATH (cd $CANN_PATH/ops_base && pwd)
        if test "$param_mult_ver" = "multi_version"
            set _ASCEND_OPS_BASE_PATH (cd $INSATLL_PATH/latest/ops_base && pwd)
        end
    end
elseif test -d "$CANN_PATH/ops_base"
    set _ASCEND_OPS_BASE_PATH (cd $CANN_PATH/ops_base && pwd)
end
