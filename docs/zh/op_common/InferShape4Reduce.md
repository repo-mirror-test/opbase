# InferShape4Reduce<a name="ZH-CN_TOPIC_0000002450096166"></a>
...
图模式场景下，reduce类算子的InferShape方法（用于推导输出张量的形状shape）。
...
```
// IsFinite算子进行inferShape推导，其推导过程与Reduce算子推导过程一致，可直接复用
